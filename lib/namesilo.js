...
const LIVE_API_URL = 'https://www.namesilo.com/'
const SANDBOX_API_URL = 'https://sandbox.namesilo.com/'
 ...
 * @param  {Boolean} [options.batch=false] Use Batch API
...
class NameSilo {
  constructor (options) {
    ...
    this.setHTTPClient(this.createHTTPClient(options))
    this.attachMethods()
  }
  ...
  createHTTPClient (options) {
    let client = axios.create({
      baseURL:  `${options.sandbox ? SANDBOX_API_URL : LIVE_API_URL}${options.batch ? 'apibatch' : 'api'}` ,
      params: {
  ...
  attachMethods () {
    for (let action of Object.keys(actions)) {
      ...
      this[action] = function (options) {
        ...
        return this.get(action, options)
      }
    }
  }

  /**
   * Send a wrapped GET request to the NameSilo API.
   ...
   * @example
   * let response = await client.get('registerDomain', {
   *   domain: 'example.com',
   *   years: 2
   * }, (data) => {
   *   // ...
   ...
   * })
   */
  async get (action, inputs, parser) {
    ...
    return new Promise((resolve, reject) => {
      this.axios.get(action, { params: inputs })
        .then((response) => {
          ...
          Object.defineProperty(data, 'success', {
            get () { return this.code && this.code >= 300 && this.code < 400 }
          })

          Object.defineProperty(data, 'failed', {
            get () { return !this.code || this.code < 300 || this.code >= 400 }
          })
  ...
  checkRegisterAvailability (domains) {
    ...
    return this.get('checkRegisterAvailability', { domains }, (data) => {
      ...
      if (data.available) {
        for (let i=0; i < data.available.length; i++) {
          if (utils.isString(data.available[i])) {
            data.available[i] = { domain: data.available[i], price: undefined }
          } else {
  ...
  changeNameServers (domain, nameservers) {
    ...
    return this.get('changeNameServers', options)
  }
   ...
   */
  listDomains () {
    return this.get('listDomains', (data) => {
   ...
   * @param {Object} [options] See https://www.namesilo.com/api_reference.php#registerDomain
   * @return {Promise} API Reply
   * @example
  ...
  registerDomain (domain, years=1, options) {
    ...
    // TODO(TEST): Allows passing of ['ns1.example.com', 'ns2.example.com']
    if (utils.isArray(options.nameservers)) {
      for (let i=0; i < options.nameservers.length; i++) {
        options[`ns${i+1}`] = options.nameservers[i]
      }
      delete options.nameservers
    }

    return this.get('registerDomain', options)
  }
   ...
   * @param {Number} [years=1] The amount of years to renew domain for
  ...
  renewDomain (domain, years=1, options) {
    ...
    return this.get('renewDomain', options)
  }
  ...
  //   return this.get('getDomainInfo', options)
  // }
}
